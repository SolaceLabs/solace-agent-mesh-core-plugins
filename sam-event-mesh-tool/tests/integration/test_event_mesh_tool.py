...
async def test_parameter_defaults_and_overrides():
    ...
    assert payload_defaults == expected_defaults, f"Expected {expected_defaults}, got {payload_defaults}"
    ...
    assert payload_partial == expected_partial, f"Expected {expected_partial}, got {payload_partial}"
...
async def test_missing_required_parameters():
    ...
    assert "required_param" in schema.required, "required_param should be in the required list"
    assert "optional_param" not in schema.required, "optional_param should not be in the required list"
    
    # Verify the schema has the correct properties
    assert "required_param" in schema.properties, "required_param should be in properties"
    assert "optional_param" in schema.properties, "optional_param should be in properties"
...
async def test_parameter_type_validation():
    ...
    assert schema.properties["unknown_type_param"].description == "A parameter with unknown type (should default to string)"
