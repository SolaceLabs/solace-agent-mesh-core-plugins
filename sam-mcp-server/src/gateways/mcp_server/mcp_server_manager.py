...
class MCPServerManager:
        ...
        with self.lock:
            ...
            try:
                ...
                self.server = MCPServerFactory.get_server(
                    self.server_name, self.host, self.port, self.transport_type
                )
        ...
        try:
            ...
            if not agent:
                return CallToolResult(
                    content=[
                        TextContent(type="text", text=f"Agent '{agent_name}' not found")
                    ],
                    isError=True,
                )
            ...
            if not action:
                return CallToolResult(
                    content=[
                        TextContent(
                            type="text",
                            text=f"Action '{action_name}' not found for agent '{agent_name}'",
                        )
                    ],
                    isError=True,
                )
                ...
                if param_required and param_name not in args:
                    return CallToolResult(
                        content=[
                            TextContent(
                                type="text",
                                text=f"Missing required parameter '{param_name}'",
                            )
                        ],
                        isError=True,
                    )
            ...
            log.info(
                f"{self.log_identifier}Sending action request to {agent_name}.{action_name} with correlation ID {correlation_id}"
            )
                ...
                try:
                    ...
                    return CallToolResult(
                        content=[
                            TextContent(
                                type="text",
                                text=f"Request to {agent_name}.{action_name} timed out after {timeout_seconds} seconds",
                            )
                        ],
                        isError=True,
                    )
    ...
    def _handle_resource_read(
        self, agent_name: str, resource_uri: str
    ) -> ReadResourceResult:
        ...
        try:
            ...
            try:
                ...
                return ReadResourceResult(contents=[resource_content])
    ...
    def _handle_prompt_get(
        self, agent_name: str, prompt_name: str, args: Dict[str, Any]
    ) -> GetPromptResult:
        ...
        try:
            ...
            # Return the result
            return GetPromptResult(
                messages=messages, description=prompt.get("description", "")
            )
        except Exception as e:
            log.error(
            ...
            return GetPromptResult(messages=[], description=f"Error: {str(e)}")

    def handle_action_response(
        self, correlation_id: str, response_data: Dict[str, Any]
    ) -> bool:
        ...
        with self.lock:
            ...
            if correlation_id not in self.pending_requests:
                log.warning(
                    f"{self.log_identifier}Received response for unknown correlation ID: {correlation_id}"
                )
                return False
    ...
    def authenticate_client(
        ...
        try:
            ...
            if session:
                log.info(
                    f"{self.log_identifier}Client {client_id} authenticated successfully"
                )
                return session.session_id
            else:
                log.warning(
                    f"{self.log_identifier}Authentication failed for client {client_id}"
                )
                return None
        except Exception as e:
            log.error(
        ...
        with self.lock:
            ...
            for correlation_id, request_info in list(self.pending_requests.items()):
                if current_time - request_info["timestamp"] > max_age_seconds:
                    ...
                    log.warning(
                        f"{self.log_identifier}Request to {agent_name}.{action_name} timed out after {max_age_seconds} seconds"
                    )
            ...
            if expired_sessions:
                log.info(
                    f"{self.log_identifier}Cleaned up {len(expired_sessions)} expired sessions"
                )
