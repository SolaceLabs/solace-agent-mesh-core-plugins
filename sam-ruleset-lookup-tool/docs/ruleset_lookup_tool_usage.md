...
### 3. Comprehensive Groups
```yaml
✅ Good:
decision_tree_groups:
  netherlands: { ... }
  germany: { ... }
  _default: { ... }  # Always include default
...
### 4. Validation-Friendly Logic
```
✅ Good:
IF employeeGroup == 'internal employee' AND jobGrade IN ['CT1', 'CT2']:
  RETURN 'NLD_LOC_C&T'

❌ Avoid:
IF employee is coffee & tea worker:
  RETURN some_label
```

## Integration with Agent Instructions

When using this tool in an agent, structure your instructions to:

1. **Identify** when to use the tool
2. **Extract** the grouping parameter from context
3. **Evaluate** the returned logic
4. **Apply** the results appropriately

Example agent instruction:
```yaml
instruction: |
  When answering policy questions:
  1. Call get_rulesets_hr_policies with the user's country
  2. Read the returned decision_logic for the relevant topic
