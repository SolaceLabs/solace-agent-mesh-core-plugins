...
class TestA2AClientActionInvokeMapping(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_params_def = [
            ...
            {
                "name": "files",
                "desc": "Optional files (JSON list of URLs)",
                "type": "string",
                "required": False,
            },
        ]
        ...
        self.text_part_patcher = patch(
            "src.agents.a2a_client.actions.a2a_client_action.TextPart", wraps=TextPart
        )
        self.file_part_patcher = patch(
            "src.agents.a2a_client.actions.a2a_client_action.FilePart", wraps=FilePart
        )
        self.file_content_patcher = patch(
    ...
    def test_invoke_mapping_text_and_one_file(self):
        ...
        self.assertEqual(
            sent_payload["message"]["parts"][1]["file"]["name"], "file1.txt"
        )
        self.assertEqual(
            sent_payload["message"]["parts"][1]["file"]["bytes"], expected_b64
        )
    ...
    def test_invoke_mapping_text_and_multiple_files(self):
        ...
        params = {
            ...
            "files": json.dumps([file_url1, file_url2]),
        }
        self.action.invoke(params, self.meta)
        ...
        file_names = {
            part.file.name for part in sent_parts if isinstance(part, FilePart)
        }
